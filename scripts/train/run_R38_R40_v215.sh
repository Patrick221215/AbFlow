...
usage() {
  cat >&2 <<'USAGE'
Usage:
  bash scripts/train/run_R38_R40_v215.sh <config.json> \
...
if 'gpus' in runtime:
    raise SystemExit(
        "runtime.gpus is forbidden in V215 configs; pass physical GPUs with --gpus"
    )
output = training['output_dir']
if not os.path.isabs(output):
    output = os.path.abspath(os.path.join(root, output))
# Backward-compatible fallback only. Formal V215 configs leave this empty.
...
if not exp_id:
    raise SystemExit('V215 identity preflight failed: experiment.id is missing')
if not (exp_id == config_stem == output_id):
    raise SystemExit(
        'V215 identity preflight failed:\n'
...
if generation_dir:
    ...
    if generation_id != exp_id:
        raise SystemExit(
            'V215 generation provenance preflight failed:\n'
            f'  experiment.id={exp_id}\n'
...
if ! grep -q "GradientNormOverflowRecovered" "$PROJECT_ROOT/trainer/abs_trainer.py"; then
  echo "V215 requires the already-installed V10 stable finite-gradient norm fallback in trainer/abs_trainer.py" >&2
  exit 2
fi
...
for rel in [
    ...
    'train.py',
    'scripts/train/run_R38_R40_v215.sh',
]:
...
pc_mode=str(pc.get('mode','bounded_residual') or 'bounded_residual').strip().lower()
if pc_mode != 'bounded_residual':
    raise SystemExit(
        f'V215 requires pair_coordinate.mode=bounded_residual; got {pc_mode!r}'
    )
delta_bound=float(pc.get('delta_bound',1.0))
if delta_bound <= 0:
    raise SystemExit('V215 requires pair_coordinate.delta_bound > 0')

cc_mode=str(cc.get('mode','') or '').strip().lower()
if cc_mode != 'egnn_tanh_normalized':
    raise SystemExit(
        'V215 requires coordinate_controller.mode=egnn_tanh_normalized; '
        f'got {cc_mode!r}'
    )
...
if sl_source not in allowed:
    raise SystemExit(f'V215 invalid smooth-lDDT prediction_source={sl_source!r}; allowed={sorted(allowed)}')

print(
    '[PairCoordinateContract] '
    f'mode={pc_mode} delta_bound={delta_bound:g} '
    'state_pair=full coordinate_pair=bounded_residual'
)
print(
    '[CoordinateControllerContract] '
    f'mode={cc_mode} base_scalar_activation=tanh relative_vector=unit_direction_detached '
    'base_scalar_abs_bound=1 '
    f'pair_delta_abs_bound={delta_bound:g} '
    'effective_scalar_abs_bound<=1 '
    'raw_distance_features=preserved direction_norm_bound<=1 no_coordinate_clipping=1'
)
print(
    '[SmoothLDDTContract] '
