...
usage() {
  cat >&2 <<'USAGE'
Usage:
  bash scripts/train/run_R47_R49_v218.sh <config.json> \
...
if 'gpus' in runtime:
    raise SystemExit(
        "runtime.gpus is forbidden in V218 configs; pass physical GPUs with --gpus"
    )
output = training['output_dir']
if not os.path.isabs(output):
    output = os.path.abspath(os.path.join(root, output))
# Backward-compatible fallback only. Formal V218 configs leave this empty.
...
if not exp_id:
    raise SystemExit('V218 identity preflight failed: experiment.id is missing')
if not (exp_id == config_stem == output_id):
    raise SystemExit(
        'V218 identity preflight failed:\n'
...
if generation_dir:
    ...
    if generation_id != exp_id:
        raise SystemExit(
            'V218 generation provenance preflight failed:\n'
            f'  experiment.id={exp_id}\n'
...
if ! grep -q "GradientNormOverflowRecovered" "$PROJECT_ROOT/trainer/abs_trainer.py"; then
  echo "V218 requires the already-installed V10 stable finite-gradient norm fallback in trainer/abs_trainer.py" >&2
  exit 2
fi
...
for rel in [
    ...
    'train.py',
    'scripts/train/run_R47_R49_v218.sh',
]:
...
if pc_mode != 'direct_shared':
    raise SystemExit(
        f"V218 requires pair_coordinate.mode='direct_shared'; got {pc_mode!r}"
    )
if 'delta_bound' in pc:
    raise SystemExit(
        'V218 direct_shared must not carry stale pair_coordinate.delta_bound; '
        'there is no bounded Pair residual in the formal controller.'
    )

cc_mode=str(cc.get('mode','') or '').strip().lower()
if cc_mode != 'egnn_unit_direction':
    raise SystemExit(
        'V218 requires coordinate_controller.mode=egnn_unit_direction; '
        f'got {cc_mode!r}'
    )
...
if sl_source not in allowed:
    raise SystemExit(f'V218 invalid smooth-lDDT prediction_source={sl_source!r}; allowed={sorted(allowed)}')
...
print(
    ...
    'relative_vector=unit_direction_differentiable raw_distance_features=preserved '
