...
# ============================================================
# AbFlow v72: transport-space Score-Flow factorization stage
# ============================================================
# Three matched formal experiments (config stem = EXP_ID = run folder):
#   F01_PCS_RC_LC_R1_SF2M_R3_CANONICAL_FLOW
#       endpoint-free exact stochastic conditional probability-flow target.
#   F02_PCS_RC_LC_R1_SF2M_R3_TIED_SCORE_CONTROL
#       same Flow + analytically tied conditional-score consistency.
#   F03_PCS_RC_LC_R1_SF2M_R3_DUALFIELD_SCORE_FLOW
#       same Flow + independent E(3)-equivariant global-R3 score field.
#
# All three keep the same PCS-RC source, recurrent proposal context, F01
# task-adaptive global-R3 corruption path, full-atom Cartesian backbone and
# formal probability-flow ODE sampler.  The special clean-endpoint coordinate
# objective has zero training weight in every formal v71 mode.
#
# We deliberately do NOT claim an exact Schrodinger bridge: unlike the AISTATS
# SF2M SB specialization, AbFlow already has observed proposal-native pairing
# and uses pair-adaptive R3 width rather than entropic OT + a common Brownian
# reference diffusion.
# ============================================================
STATE_PATH=on
PER_SAMPLE_T=on
...
R3_SCORE_DSM_WEIGHT=${ABFLOW_R3_SCORE_DSM_WEIGHT:-0.02}
R3_PATHFLOW_WEIGHT=${ABFLOW_R3_PATHFLOW_WEIGHT:-0.02}
R3_FLOW_COORDINATE_SCALING=${ABFLOW_R3_FLOW_COORDINATE_SCALING:-0.1}
SF2M_SCORE_WEIGHT=${ABFLOW_SF2M_SCORE_WEIGHT:-1.0}
...
case "$EXP_ID" in
  F01_PCS_RC_LC_R1_SF2M_R3_CANONICAL_FLOW)
    ABLATION_PARENT=PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT
    EXPERIMENT_FACTOR=endpoint_free_exact_stochastic_probability_flow
    MODULE_ID=F01_SF2M_R3_CANONICAL_FLOW
    MODULE_PARENT=F01_GLOBAL_R3_PATH
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=sf2m_r3_canonical_flow
    T_SAMPLING=sf2m_uniform
    SAMPLER_MODE=sf2m_ode
  ...
  F02_PCS_RC_LC_R1_SF2M_R3_MEAN_FLOW_CONTROL)
    ABLATION_PARENT=F01_PCS_RC_LC_R1_SF2M_R3_CANONICAL_FLOW
    EXPERIMENT_FACTOR=remove_stochastic_velocity_correction_keep_same_states
    MODULE_ID=F02_SF2M_R3_MEAN_FLOW_CONTROL
    MODULE_PARENT=F01_SF2M_R3_CANONICAL_FLOW
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=sf2m_r3_mean_flow_control
    T_SAMPLING=sf2m_uniform
    SAMPLER_MODE=sf2m_ode
    COORD_PEP_AS_CONDITION=on
    SEQ_INPUT_MODE=pep_condition
  ...
  F03_PCS_RC_LC_R1_SF2M_R3_DECOMPOSED_SCORE_FLOW)
    ABLATION_PARENT=F02_PCS_RC_LC_R1_SF2M_R3_MEAN_FLOW_CONTROL
    EXPERIMENT_FACTOR=exact_score_induced_flow_correction_factorization
    MODULE_ID=F03_SF2M_R3_DECOMPOSED_SCORE_FLOW
    MODULE_PARENT=F02_SF2M_R3_MEAN_FLOW_CONTROL
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=sf2m_r3_decomposed_score_flow
    T_SAMPLING=sf2m_uniform
    SAMPLER_MODE=sf2m_decomposed_ode
    COORD_PEP_AS_CONDITION=on
    SEQ_INPUT_MODE=pep_condition
    SHADOW_SEQ_STATE=off
    DUAL_SEQUENCE_STATE=off
  ...
  *)
    ...
    echo "Supported v72 experiments:"
    echo "  F01_PCS_RC_LC_R1_SF2M_R3_CANONICAL_FLOW"
    echo "  F02_PCS_RC_LC_R1_SF2M_R3_MEAN_FLOW_CONTROL"
    echo "  F03_PCS_RC_LC_R1_SF2M_R3_DECOMPOSED_SCORE_FLOW"
    exit 2
    ;;
esac
...
if [[ "$MODE" != "train" ]]; then
  ...
  echo "v71 EXP_ID: F01_PCS_RC_LC_R1_SF2M_R3_CANONICAL_FLOW, F02_PCS_RC_LC_R1_SF2M_R3_TIED_SCORE_CONTROL, F03_PCS_RC_LC_R1_SF2M_R3_DUALFIELD_SCORE_FLOW"
  exit 2
fi
...
runtime = {
    ...
    "independent_score_head": os.environ.get("ABFLOW_SCOREFM_LOSS_MODE", "") == "sf2m_r3_dualfield_score_flow",
