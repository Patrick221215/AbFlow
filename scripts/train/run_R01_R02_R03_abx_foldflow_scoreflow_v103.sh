...
# ============================================================
# AbFlow v103: AbX common frame -> FoldFlow CFM -> Score--Flow
# ============================================================
# R01: AbX common-centering + exact U02 global-R3 canonical Score--Flow.
# R02: same AbX frame/PCS-RC source, deterministic full-atom Cartesian CFM.
# R03: same direct velocity decoder/sampler as R02, plus a protein-aware
#      residue-translation stochastic bridge and canonical Score correction.
#
# All three keep the SAME AbFlow architecture, PCS-RC source, recurrent
# proposal context, sequence task, optimizer/runtime settings and evaluation.
# No Pair-Time, SATC, PCGrad, support-factorized loss, round credit, extra head,
# independent Score head or independent Flow head is enabled.
# ============================================================
...
# AbX standard: antibody backbone CA center is the ONE common translation
# applied to antibody, antigen, surface, PCS-RC source, target and Xt.
export ABFLOW_ABX_COMMON_CENTER="on"

# FoldFlow numerical convention: model coordinates are Angstrom/10.
export ABFLOW_R3_FLOW_COORDINATE_SCALING="0.1"

# U02 proposal/context + original sequence semantics are frozen.
...
# Disable historical branches that are outside the v103 question.
...
# Historical F01 knobs remain fixed for R01 only. R02/R03 direct modes do not
# consume adaptive source/native g.
...
case "$EXP_ID" in
  R01_PCS_RC_LC_R1_ABX_U02_GLOBAL_R3_SCOREFLOW)
    export ABFLOW_ABLATION_PARENT="U02_PCS_RC_LC_R1_FF_R3_ENDPOINT_CANONICAL_HYBRID"
    export ABFLOW_EXPERIMENT_FACTOR="abx_common_center_only_on_u02"
    export ABFLOW_SINGLE_FACTOR_ABLATION="true"
    export ABFLOW_MODULE_ID="R01_ABX_U02"
  ...
  R02_PCS_RC_LC_R1_ABX_CARTESIAN_CFM)
    export ABFLOW_ABLATION_PARENT="R01_PCS_RC_LC_R1_ABX_U02_GLOBAL_R3_SCOREFLOW"
    export ABFLOW_EXPERIMENT_FACTOR="replace_u02_stochastic_carrier_with_direct_cartesian_cfm"
    export ABFLOW_SINGLE_FACTOR_ABLATION="false"
    export ABFLOW_MODULE_ID="R02_ABX_CARTESIAN_CFM"
    export ABFLOW_MODULE_PARENT="R01_ABX_U02"

    # FoldFlow-base algebra under our paired PCS-RC->native coupling:
    #   Xt=(1-t)X0+tX1, u*=X1-X0, v_theta=Y-Xt, deterministic ODE.
    export ABFLOW_SCOREFM_LOSS_MODE="abx_cartesian_cfm"
  ...
  R03_PCS_RC_LC_R1_ABX_CARTESIAN_SCOREFLOW)
    ...
    # Same direct decoder and same deterministic probability-flow ODE as R02.
    # Only the training path/target gains a FoldFlow-inspired per-residue R3
    # stochastic residual and the analytically matched canonical Score term.
    export ABFLOW_SCOREFM_LOSS_MODE="abx_cartesian_scoreflow"
    export ABFLOW_SCOREFM_SAMPLER_MODE="cartesian_direct_ode"
    export ABFLOW_FLOW_T_MIN="0.01"
    export ABFLOW_FLOW_T_MAX="0.99"
    export ABFLOW_R03_G_SCALED="0.1"
    # Clean paired endpoints are a task requirement; unlike FoldFlow's optional
    # SFM floor, the physical bridge floor is zero. Score numerical floor stays 0.01.
    export ABFLOW_R03_PATH_MIN_SIGMA_SCALED="0.0"
    # Conditional FoldFlow adaptation: center the stochastic translation
    # residual, not the H3 mean state, so antigen-relative placement is retained.
    export ABFLOW_R03_CENTER_RESIDUAL="on"
    ;;
  ...
  *)
    ...
    echo "Supported v103:"
    echo "  R01_PCS_RC_LC_R1_ABX_U02_GLOBAL_R3_SCOREFLOW"
    echo "  R02_PCS_RC_LC_R1_ABX_CARTESIAN_CFM"
    echo "  R03_PCS_RC_LC_R1_ABX_CARTESIAN_SCOREFLOW"
    exit 2
    ;;
esac
