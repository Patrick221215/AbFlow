...
case "$EXP_ID" in
  ...
  PCS_RC_LC_R1_STRUCT_GLOBAL_FFSQRT_ENDPOINT|S04_STRUCT_GLOBAL_FFSQRT_ENDPOINT)
    ABLATION_PARENT=PCS_RC_LC_R1_STRUCT_GLOBAL_ENDPOINT
    EXPERIMENT_FACTOR=foldflow_sqrt_time_profile
    MODULE_ID=S04_STRUCT_GLOBAL_FFSQRT_ENDPOINT
    MODULE_PARENT=S01_STRUCT_GLOBAL_ENDPOINT
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=structured_global_ffsqrt_endpoint
    T_SAMPLING=uniform
    SAMPLER_MODE=bridge
    COORD_PEP_AS_CONDITION=on
    SEQ_INPUT_MODE=pep_condition
  ...
  PCS_RC_LC_R1_STRUCT_MULTISCALE_ENDPOINT|S05_STRUCT_MULTISCALE_ENDPOINT)
    ABLATION_PARENT=PCS_RC_LC_R1_STRUCT_GLOBAL_ENDPOINT
    EXPERIMENT_FACTOR=orthogonal_local_path_with_clean_endpoint
    MODULE_ID=S05_STRUCT_MULTISCALE_ENDPOINT
    MODULE_PARENT=S01_STRUCT_GLOBAL_ENDPOINT
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=structured_multiscale_endpoint
    T_SAMPLING=uniform
    SAMPLER_MODE=bridge
    COORD_PEP_AS_CONDITION=on
    SEQ_INPUT_MODE=pep_condition
  ...
  *)
    ...
    echo "  PCS_RC_LC_R1_STRUCT_GLOBAL_FFSQRT_ENDPOINT"
    echo "  PCS_RC_LC_R1_STRUCT_MULTISCALE_ENDPOINT"
    exit 2
    ;;
esac
...
if [[ "$MODE" != "train" ]]; then
  ...
  echo "Structured-v58 EXP_ID: PCS_RC_LC_R1_STRUCT_GLOBAL_ENDPOINT, PCS_RC_LC_R1_STRUCT_GLOBAL_FFSQRT_ENDPOINT, PCS_RC_LC_R1_STRUCT_MULTISCALE_ENDPOINT"
  exit 2
fi
