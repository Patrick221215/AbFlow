...
usage() {
  cat >&2 <<'USAGE'
Usage:
  bash scripts/train/run_R50_R52_v219.sh <config.json> \
...
if 'gpus' in runtime:
    raise SystemExit(
        "runtime.gpus is forbidden in V219 configs; pass physical GPUs with --gpus"
    )
output = training['output_dir']
if not os.path.isabs(output):
    output = os.path.abspath(os.path.join(root, output))
# Backward-compatible fallback only. Formal V219 configs leave this empty.
...
if not exp_id:
    raise SystemExit('V219 identity preflight failed: experiment.id is missing')
if not (exp_id == config_stem == output_id):
    raise SystemExit(
        'V219 identity preflight failed:\n'
...
if generation_dir:
    ...
    if generation_id != exp_id:
        raise SystemExit(
            'V219 generation provenance preflight failed:\n'
            f'  experiment.id={exp_id}\n'
...
if ! grep -q "GradientNormOverflowRecovered" "$PROJECT_ROOT/trainer/abs_trainer.py"; then
  echo "V219 requires the already-installed V10 stable finite-gradient norm fallback in trainer/abs_trainer.py" >&2
  exit 2
fi
...
for rel in [
    ...
    'train.py',
    'scripts/train/run_R50_R52_v219.sh',
]:
...
if pc_mode != 'direct_shared':
    raise SystemExit(
        f"V219 requires pair_coordinate.mode='direct_shared'; got {pc_mode!r}"
    )
if 'delta_bound' in pc:
    raise SystemExit(
        'V219 direct_shared must not carry stale pair_coordinate.delta_bound; '
...
if cc_mode != 'egnn_prenorm_raw':
    raise SystemExit(
        'V219 requires coordinate_controller.mode=egnn_prenorm_raw; '
        f'got {cc_mode!r}'
    )
...
if sl_source not in allowed:
    raise SystemExit(f'V219 invalid smooth-lDDT prediction_source={sl_source!r}; allowed={sorted(allowed)}')
