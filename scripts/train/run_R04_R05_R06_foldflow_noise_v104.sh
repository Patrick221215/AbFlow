...
# R05MF_GEOM_CALIBRATION_V179: R26 = R25 representation + live pair distogram calibration.
set -euo pipefail
...
export ABFLOW_R05_GEOM_DISTOGRAM="off"
export ABFLOW_LOSS_R05_GEOM_DISTOGRAM_WEIGHT="0.0"
export ABFLOW_R05_GEOM_TRIANGLE_ATTENTION="off"  # legacy compatibility only
export ABFLOW_R05_GEOM_TRIANGLE_MODE="multiplication"
...
case "$EXP_ID" in
  ...
  R23_R05_ABX_PAIR_TIME_U02|R24_R05_GEOMETRY_PAIR_TRIATTN_U02|R25_R05_GEOMETRY_PAIR_U02|R26_R05_GEOMETRY_PAIR_LIVE_DISTOGRAM_U02)
    export ABFLOW_ABLATION_PARENT="R05_PCS_RC_LC_R1_ABX_FF_FIXEDG_RESIDUE_U02_SCOREFLOW"
    ...
    case "$EXP_ID" in
      ...
      R24_R05_GEOMETRY_PAIR_TRIATTN_U02)
        # v177 replacement R24 is a DIRECT child of R05.  It builds the same
        # minimal CA geometry-pair state as R25, but uses AF2 starting/ending
        # triangle ATTENTION as the only higher-order pair operator.
        export ABFLOW_R05_GEOM_PAIR="on"
        export ABFLOW_R05_GEOM_TRIANGLE_MODE="attention"
        export ABFLOW_DDP_FIND_UNUSED_PARAMETERS="off" ;;
      R25_R05_GEOMETRY_PAIR_U02)
        # R25 remains the direct-R05 triangle MULTIPLICATION sibling.
        export ABFLOW_R05_GEOM_PAIR="on"
        export ABFLOW_R05_GEOM_TRIANGLE_MODE="multiplication"
        export ABFLOW_DDP_FIND_UNUSED_PARAMETERS="on" ;;
      R26_R05_GEOMETRY_PAIR_LIVE_DISTOGRAM_U02)
        # V179 R25-successor.  The pair/triangle/EGNN route is identical to R25;
        # only a live final-pair native-distance calibration objective is added.
        export ABFLOW_R05_GEOM_PAIR="on"
        export ABFLOW_R05_GEOM_TRIANGLE_MODE="multiplication"
        export ABFLOW_R05_GEOM_DISTOGRAM="on"
        export ABFLOW_LOSS_R05_GEOM_DISTOGRAM_WEIGHT="0.03"
        export ABFLOW_DDP_FIND_UNUSED_PARAMETERS="on" ;;
    esac
    ;;
  ...
  *)
    ...
    echo "  R23_R05_ABX_PAIR_TIME_U02"
    echo "  R24_R05_GEOMETRY_PAIR_TRIATTN_U02"
    echo "  R25_R05_GEOMETRY_PAIR_U02"
    echo "  R26_R05_GEOMETRY_PAIR_LIVE_DISTOGRAM_U02"
    exit 2
    ;;
esac
...
case "$EXP_ID" in
  ...
  R24_R05_GEOMETRY_PAIR_TRIATTN_U02) _ABFLOW_EXPECTED_GPUS="4,5" ;;
  R25_R05_GEOMETRY_PAIR_U02) _ABFLOW_EXPECTED_GPUS="6,7" ;;
  R26_R05_GEOMETRY_PAIR_LIVE_DISTOGRAM_U02) _ABFLOW_EXPECTED_GPUS="6,7" ;;
...
print_settings() {
  ...
  echo "R05_GEOM_DISTOGRAM=$ABFLOW_R05_GEOM_DISTOGRAM"
  echo "LOSS_R05_GEOM_DISTOGRAM_WEIGHT=$ABFLOW_LOSS_R05_GEOM_DISTOGRAM_WEIGHT"
  echo "R05_GEOM_TRIANGLE_MODE=$ABFLOW_R05_GEOM_TRIANGLE_MODE"
...
for key, value in env.items():
    if not (
        ...
        or key in {'ABFLOW_R05_ENDPOINT_RELATION', 'ABFLOW_R05_GEOM_PAIR', 'ABFLOW_R05_GEOM_DISTOGRAM', 'ABFLOW_R05_GEOM_TRIANGLE_ATTENTION', 'ABFLOW_R05_GEOM_TRIANGLE_MODE', 'ABFLOW_PAIR_TIME_SCOPE'}
    ):
        raise SystemExit(
...
case "$EXP_ID" in
  R23_R05_ABX_PAIR_TIME_U02|R24_R05_GEOMETRY_PAIR_TRIATTN_U02|R25_R05_GEOMETRY_PAIR_U02|R26_R05_GEOMETRY_PAIR_LIVE_DISTOGRAM_U02)
    ...
    [[ "$ABFLOW_SEQUENCE_CONTEXT_MODE" == "legacy" ]] || { echo "ERROR: V179 freezes original AbFlow sequence context." >&2; exit 2; }
    _is_on "$ABFLOW_EPOCH_TEST" || { echo "ERROR: V179 preserves Epoch Test ON." >&2; exit 2; }
    case "$EXP_ID" in
      R23_R05_ABX_PAIR_TIME_U02)
      ...
      R24_R05_GEOMETRY_PAIR_TRIATTN_U02)
        ...
        [[ "$ABFLOW_R05_GEOM_TRIANGLE_MODE" == "attention" ]] || exit 2
        [[ "$ABFLOW_MF_SMOOTH_LDDT" == "off" && "$ABFLOW_LOSS_SMOOTH_LDDT_WEIGHT" == "0.0" ]] || exit 2 ;;
      R25_R05_GEOMETRY_PAIR_U02)
        [[ "$ABFLOW_PAIR_TIME_SCOPE" == "off" && "$ABFLOW_R05_ENDPOINT_RELATION" == "off" && "$ABFLOW_R05_GEOM_PAIR" == "on" ]] || exit 2
        [[ "$ABFLOW_R05_GEOM_TRIANGLE_MODE" == "multiplication" ]] || exit 2
        [[ "$ABFLOW_R05_GEOM_DISTOGRAM" == "off" && "$ABFLOW_LOSS_R05_GEOM_DISTOGRAM_WEIGHT" == "0.0" ]] || exit 2 ;;
      R26_R05_GEOMETRY_PAIR_LIVE_DISTOGRAM_U02)
        [[ "$ABFLOW_PAIR_TIME_SCOPE" == "off" && "$ABFLOW_R05_ENDPOINT_RELATION" == "off" && "$ABFLOW_R05_GEOM_PAIR" == "on" ]] || exit 2
        [[ "$ABFLOW_R05_GEOM_TRIANGLE_MODE" == "multiplication" ]] || exit 2
        [[ "$ABFLOW_R05_GEOM_DISTOGRAM" == "on" && "$ABFLOW_LOSS_R05_GEOM_DISTOGRAM_WEIGHT" == "0.03" ]] || exit 2
        [[ "$ABFLOW_MF_DISTOGRAM" == "off" && "$ABFLOW_LOSS_DISTOGRAM_WEIGHT" == "0.0" ]] || exit 2 ;;
    esac
    if [[ "$EXP_ID" == "R26_R05_GEOMETRY_PAIR_LIVE_DISTOGRAM_U02" ]]; then
      grep -q "R05MF_GEOM_CALIBRATION_V179" "$PROJECT_ROOT/models/AbFlow/AbFlow_model.py" || { echo "ERROR: R26 requires v179 model overlay." >&2; exit 2; }
      grep -q "def distogram_loss(self, true_local_X, pair_edges, pair_state, local_is_ab)" "$PROJECT_ROOT/models/AbFlow/AbFlow_model.py" || { echo "ERROR: R26 live pair distogram head missing." >&2; exit 2; }
    fi
    echo "[V179Preflight] direct-R05 physics + geometry-calibration contract PASS: $EXP_ID"
    ;;
esac
...
case "$EXP_ID" in
  R23_R05_ABX_PAIR_TIME_U02|R24_R05_GEOMETRY_PAIR_TRIATTN_U02|R25_R05_GEOMETRY_PAIR_U02|R26_R05_GEOMETRY_PAIR_LIVE_DISTOGRAM_U02)
    python "$PROJECT_ROOT/scripts/train/validate_R05_v179.py" --root "$PROJECT_ROOT" --config "$BASE_CONFIG" --runtime
    python "$PROJECT_ROOT/scripts/train/validate_R05_v179.py" --root "$PROJECT_ROOT" --tensor
    ;;
esac
...
runtime = {
    ...
    "r05_geom_pair": os.environ.get("ABFLOW_R05_GEOM_PAIR", "off"),
    "r05_geom_triangle_mode": os.environ.get("ABFLOW_R05_GEOM_TRIANGLE_MODE", "multiplication"),
    "r05_geom_distogram": os.environ.get("ABFLOW_R05_GEOM_DISTOGRAM", "off"),
    "loss_r05_geom_distogram_weight": os.environ.get("ABFLOW_LOSS_R05_GEOM_DISTOGRAM_WEIGHT", "0.0"),
