...
usage() {
  cat >&2 <<'USAGE'
Usage:
  bash scripts/train/run_R31_R32_R33_v212.sh <config.json> \
...
if 'gpus' in runtime:
    raise SystemExit(
        "runtime.gpus is forbidden in V212 configs; pass physical GPUs with --gpus"
    )
output = training['output_dir']
if not os.path.isabs(output):
    output = os.path.abspath(os.path.join(root, output))
# Backward-compatible fallback only. Formal V212 configs leave this empty.
...
if ! grep -q "GradientNormOverflowRecovered" "$PROJECT_ROOT/trainer/abs_trainer.py"; then
  echo "V212 requires the already-installed V10 stable finite-gradient norm fallback in trainer/abs_trainer.py" >&2
  exit 2
fi
...
for rel in [
    ...
    'train.py',
    'scripts/train/run_R31_R32_R33_v212.sh',
]:
