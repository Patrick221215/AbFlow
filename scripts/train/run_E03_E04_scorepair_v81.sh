...
# ============================================================
# AbFlow v81: fixed-g matched Endpoint Score--Flow + analytic Score-conditioned pair refinement
# ============================================================
# Three scientific configurations (config stem = EXP_ID = run folder):
#   E00_PCS_RC_LC_R1_ENDPOINT_FM_BASE
#       deterministic proposal->native conditional FM, endpoint parameterization.
#   E01_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT
#       standard Brownian-bridge-style global-R3 training states, endpoint loss.
#   E02_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT_INTERFACE_TIME
#       same stochastic endpoint path as E01 plus zero-start interface pair-time conditioning.
#   E03_PCS_RC_LC_R1_FIXEDG_MATCHED_ENDPOINT_FLOW
#       fixed inference-known Brownian width with formal current/probability-flow ODE;
#       exact SDE remains a same-checkpoint stochastic ablation.
#   E04_PCS_RC_LC_R1_FIXEDG_SCOREPAIR_SCOREFLOW
#       E01 stochastic endpoint base + interface pair conditioning by the normalized
#       Brownian uncertainty phase rho(t)=2*sqrt(t(1-t)), NOT raw time t.
#
# Training configs remain endpoint-parameterized. No independent score head and no
# pseudo-coordinate direct velocity carrier are used. E01/E02 can be evaluated from the
# SAME checkpoint with MODE=test_scoreflow or test_cosf_ode. E03 additionally supports
# MODE=test_cosf_sde.
# SATC, SO(3), SE(3), ESM2, OT and extra score/velocity losses stay off.
# ============================================================
STATE_PATH=on
PER_SAMPLE_T=on
TIME_EMBED=on
PAIR_TIME_SCOPE=${ABFLOW_PAIR_TIME_SCOPE:-off}
PAIR_TIME_SIGNAL=${ABFLOW_PAIR_TIME_SIGNAL:-time}
PAIR_SCORE_FEEDBACK=${ABFLOW_PAIR_SCORE_FEEDBACK:-off}
T_SAMPLING=uniform
LOSS_MODE=endpoint
SAMPLER_MODE=bridge
...
R3_FIXED_G=${ABFLOW_R3_FIXED_G:-0.5}
...
case "$EXP_ID" in
  E00_PCS_RC_LC_R1_ENDPOINT_FM_BASE)
    # ------------------------------------------------------------
    # E00 — matched endpoint-parameterized conditional FM base.
    # Network predicts clean X1; sampling dynamics are induced by
    # v_theta=(X1_hat-Xt)/(1-t). No stochastic R3 path, no score loss,
    # no independent score/velocity head.
    # ------------------------------------------------------------
    ABLATION_PARENT=PCS_RC_LC_R1
    EXPERIMENT_FACTOR=restore_endpoint_parameterization_and_refinement_semantics
    MODULE_ID=E00_ENDPOINT_FM_BASE
    MODULE_PARENT=PCS_RC_LC_R1
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
  ...
  E01_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT)
    # ------------------------------------------------------------
    # E01 — standard Brownian-bridge-style stochastic training path,
    # but the network STILL predicts the clean endpoint X1 and the
    # sampler remains the endpoint bridge. This isolates whether exposing
    # the endpoint predictor to principled off-mean R3 placement states helps.
    # sigma(t)=g*sqrt(t(1-t)); one R3 translation per H3 graph.
    # ------------------------------------------------------------
    ABLATION_PARENT=E00_PCS_RC_LC_R1_ENDPOINT_FM_BASE
    EXPERIMENT_FACTOR=standard_brownian_bridge_r3_training_state
    MODULE_ID=E01_STOCHASTIC_ENDPOINT
    MODULE_PARENT=E00_ENDPOINT_FM_BASE
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
  ...
  E02_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT_INTERFACE_TIME)
    # ------------------------------------------------------------
    # E02 — strengthen the stochastic Endpoint base itself rather than
    # redundantly retraining an identical sampler-only configuration.
    # The ONLY model change vs E01 is explicit time conditioning on
    # antibody-antigen interface/surface messages.  AMEncoderPairTime
    # is zero-start, so the initial function is exactly E01.
    #
    # The trained checkpoint can later be evaluated with either:
    #   MODE=test            -> endpoint bridge dynamics
    #   MODE=test_scoreflow  -> hierarchical analytic Score--Flow dynamics
    # ------------------------------------------------------------
    ABLATION_PARENT=E01_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT
    EXPERIMENT_FACTOR=interface_pair_time_on_stochastic_endpoint_base
    MODULE_ID=E02_STOCHASTIC_ENDPOINT_INTERFACE_TIME
    MODULE_PARENT=E01_STOCHASTIC_ENDPOINT
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=foldflow_r3_global_endpoint
  ...
  E03_PCS_RC_LC_R1_FIXEDG_MATCHED_ENDPOINT_FLOW)
    # ------------------------------------------------------------
    # E03 — GATED next-stage current--osmotic Score--Flow base.
    # Train a clean endpoint predictor on a standard Brownian-bridge global-R3
    # path with ONE fixed, inference-known diffusion scale g.  This removes the
    # native-dependent width from E01 while keeping clean PCS-RC and native
    # endpoints.  No score/velocity head or auxiliary score/flow loss.
    #
    # The same checkpoint supports three dynamics:
    #   formal/default  -> deterministic current/probability-flow ODE
    #   test_bridge     -> endpoint bridge control
    #   test_cosf_sde   -> exact stochastic Brownian bridge SDE
    # Run E03 only after the zero-training E01/E02 same-checkpoint Score--Flow
    # gate shows that changing the dynamics is promising.
    # ------------------------------------------------------------
    ABLATION_PARENT=E01_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT
    EXPERIMENT_FACTOR=fixedg_matched_endpoint_probability_flow
    MODULE_ID=E03_FIXEDG_MATCHED_ENDPOINT_FLOW
    MODULE_PARENT=E01_STOCHASTIC_ENDPOINT
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=fixedg_r3_brownian_endpoint
    T_SAMPLING=uniform
    SAMPLER_MODE=r3_current_osmotic_ode
    COORD_PEP_AS_CONDITION=on
    SEQ_INPUT_MODE=pep_condition
    SHADOW_SEQ_STATE=off
    DUAL_SEQUENCE_STATE=off
    ...
    PROPOSAL_ADAPTER_START_ROUND=${ABFLOW_PROPOSAL_ADAPTER_START_ROUND:-1}
    PAIR_TIME_SCOPE=off
    PAIR_TIME_SIGNAL=time
    PAIR_SCORE_FEEDBACK=off
    R3_FIXED_G=${ABFLOW_R3_FIXED_G:-0.5}
    R3_PATH_MIN_SIGMA=0.0
    R3_TRANSPORT_MAX=${ABFLOW_R3_TRANSPORT_MAX:-20.0}
    R3_SCORE_MIN_SIGMA=${ABFLOW_R3_SCORE_MIN_SIGMA:-0.01}
  ...
  E04_PCS_RC_LC_R1_FIXEDG_SCOREPAIR_SCOREFLOW)
    # ------------------------------------------------------------
    # E04 — same fixed-g matched Brownian Endpoint/Score-Flow process as E03,
    # plus analytic Score-conditioned pair self-conditioning.
    # Round 0 predicts an Endpoint normally.  From round 1 onward the previous
    # Endpoint analytically induces q=sigma*score.  Only invariant q features
    # condition antigen->antibody semantic pair messages; same-layer coordinate
    # messages are unchanged.  No Score head and no Score/Flow auxiliary loss.
    # ------------------------------------------------------------
    ABLATION_PARENT=E03_PCS_RC_LC_R1_FIXEDG_MATCHED_ENDPOINT_FLOW
    EXPERIMENT_FACTOR=endpoint_induced_score_pair_self_conditioning
    MODULE_ID=E04_FIXEDG_SCOREPAIR_SCOREFLOW
    MODULE_PARENT=E03_FIXEDG_MATCHED_ENDPOINT_FLOW
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=fixedg_r3_brownian_endpoint
  ...
  *)
    ...
    echo "Supported endpoint Score--Flow experiments:"
    echo "  E00_PCS_RC_LC_R1_ENDPOINT_FM_BASE"
    echo "  E01_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT"
    echo "  E02_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT_INTERFACE_TIME"
    echo "  E03_PCS_RC_LC_R1_FIXEDG_MATCHED_ENDPOINT_FLOW"
    echo "  E04_PCS_RC_LC_R1_FIXEDG_SCOREPAIR_SCOREFLOW"
    exit 2
    ;;
esac

# Explicit bridge control for the formal E03 current/probability-flow run.
if [[ "$MODE" == "test_bridge" ]]; then
  case "$EXP_ID" in
    E03_PCS_RC_LC_R1_FIXEDG_MATCHED_ENDPOINT_FLOW|E04_PCS_RC_LC_R1_FIXEDG_SCOREPAIR_SCOREFLOW)
      SAMPLER_MODE=bridge
      ;;
    *)
      echo "test_bridge is defined for E03/E04." >&2
      exit 2
      ;;
  esac
fi

# Same-checkpoint hierarchical Score--Flow evaluation.  This is intentionally
# NOT a training configuration: E01/E02 are trained once with endpoint loss,
# then their exact checkpoints are compared under bridge vs r3_scoreflow.
PAIR_SCORE_FEEDBACK=${PAIR_SCORE_FEEDBACK:-off}

if [[ "$MODE" == "test_scoreflow" || "$MODE" == "test_cosf_ode" ]]; then
  case "$EXP_ID" in
    E01_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT|E02_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT_INTERFACE_TIME|E03_PCS_RC_LC_R1_FIXEDG_MATCHED_ENDPOINT_FLOW|E04_PCS_RC_LC_R1_FIXEDG_SCOREPAIR_SCOREFLOW)
      if [[ "$MODE" == "test_scoreflow" ]]; then
        SAMPLER_MODE=r3_scoreflow
      else
        SAMPLER_MODE=r3_current_osmotic_ode
      fi
      ;;
    *)
      echo "$MODE is only defined for stochastic endpoint checkpoints E01/E02/E03." >&2
      exit 2
      ;;
  esac
fi

if [[ "$MODE" == "test_cosf_sde" ]]; then
  if [[ "$EXP_ID" != "E03_PCS_RC_LC_R1_FIXEDG_MATCHED_ENDPOINT_FLOW" ]]; then
    echo "test_cosf_sde requires E03 fixed-g Brownian training so diffusion g is inference-known." >&2
    exit 2
  fi
  SAMPLER_MODE=r3_current_osmotic_sde
fi
...
run_with_env() {
  ...
  ABFLOW_PAIR_TIME_SIGNAL="$PAIR_TIME_SIGNAL" \
  ABFLOW_PAIR_SCORE_FEEDBACK="$PAIR_SCORE_FEEDBACK" \
...
print_settings() {
  ...
  echo "PAIR_SCORE_FEEDBACK=$PAIR_SCORE_FEEDBACK"
  echo "PAIR_TIME_SIGNAL=$PAIR_TIME_SIGNAL"
...
#     bash scripts/train/run_E00_E01_E02_endpoint_scoreflow_v77.sh train <EXP_ID> <GPU_ID> <BASE_CONFIG>
...
if [[ "$MODE" == "test" || "$MODE" == "test_bridge" || "$MODE" == "test_scoreflow" || "$MODE" == "test_cosf_ode" || "$MODE" == "test_cosf_sde" ]]; then
  CKPT=${4:-}
  RESULT_DIR=${5:-}
...
if [[ "$MODE" != "train" ]]; then
  ...
  echo "Test ScoreFlow: bash $0 test_scoreflow <E01|E02|E03 EXP_ID> <GPU_ID> <CKPT> <RESULT_DIR> [TEST_JSON]"
  echo "Test COSF ODE:  bash $0 test_cosf_ode <E01|E02|E03 EXP_ID> <GPU_ID> <CKPT> <RESULT_DIR> [TEST_JSON]"
  echo "Test COSF SDE:  bash $0 test_cosf_sde <E03 EXP_ID> <GPU_ID> <CKPT> <RESULT_DIR> [TEST_JSON]"
  echo "Attach current training auto-eval: bash $0 attach_eval <EXP_ID> <GPU_ID|auto> [EVAL_GPU_ID|auto]"
  echo "v81 EXP_ID: E00_PCS_RC_LC_R1_ENDPOINT_FM_BASE, E01_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT, E02_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT_INTERFACE_TIME, E03_PCS_RC_LC_R1_FIXEDG_MATCHED_ENDPOINT_FLOW, E04_PCS_RC_LC_R1_FIXEDG_SCOREPAIR_SCOREFLOW"
  exit 2
fi
...
runtime = {
    ...
    "pair_time_signal": os.environ.get("ABFLOW_PAIR_TIME_SIGNAL", "time"),
    ...
    "r3_fixed_g": os.environ.get("ABFLOW_R3_FIXED_G", ""),
    ...
    "foldflow_r3_training": os.environ.get("ABFLOW_SCOREFM_LOSS_MODE", "").startswith("foldflow_r3_"),
