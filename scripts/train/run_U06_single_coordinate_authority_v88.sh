...
SUPPORTED_EXP="U06_PCS_RC_LC_R1_FF_R3_ENDPOINT_CANONICAL_SINGLE_AUTHORITY"
...
# ------------------------------------------------------------------
# U06 scientific semantics: exact U02 path/target/sampler + one change
# ------------------------------------------------------------------
export ABFLOW_ABLATION_PARENT="U02_PCS_RC_LC_R1_FF_R3_ENDPOINT_CANONICAL_HYBRID"
export ABFLOW_EXPERIMENT_FACTOR="single_coordinate_authority_no_static_paratope_xloss"
export ABFLOW_SINGLE_FACTOR_ABLATION="true"
export ABFLOW_MODULE_ID="U06_F01_ENDPOINT_CANONICAL_SINGLE_AUTHORITY"
...
# ------------------------------------------------------------------
# attach watcher to an already-running U06
# ------------------------------------------------------------------
...
if [[ "$MODE" != "train" ]]; then
  echo "Train:       bash $0 train U06_PCS_RC_LC_R1_FF_R3_ENDPOINT_CANONICAL_SINGLE_AUTHORITY 6,7 <config.json>"
  echo "Test bridge: bash $0 test  U06_PCS_RC_LC_R1_FF_R3_ENDPOINT_CANONICAL_SINGLE_AUTHORITY 0 <ckpt> <result_dir> [test.json]"
  echo "Attach eval: bash $0 attach_eval U06_PCS_RC_LC_R1_FF_R3_ENDPOINT_CANONICAL_SINGLE_AUTHORITY 0 [eval_gpu|auto]"
  exit 2
fi
...
# Crucial: unlike v88, never overwrite a non-empty resume_checkpoint.
# ------------------------------------------------------------------
...
if [[ "$RUN_MODE" == "resume" ]]; then
  ...
  # Do not silently mix a fresh run into an existing U06 directory.
