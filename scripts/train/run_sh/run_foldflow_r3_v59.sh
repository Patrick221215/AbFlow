...
case "$EXP_ID" in
  ...
  PCS_RC_LC_R1_FF_R3_RESIDUE_ENDPOINT|F02_FF_R3_RESIDUE_ENDPOINT)
    ABLATION_PARENT=PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT
    EXPERIMENT_FACTOR=residue_rigid_r3_stochastic_state
    MODULE_ID=F02_FF_R3_RESIDUE_ENDPOINT
    MODULE_PARENT=F01_FF_R3_GLOBAL_ENDPOINT
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=foldflow_r3_residue_endpoint
    T_SAMPLING=uniform
    SAMPLER_MODE=bridge
    COORD_PEP_AS_CONDITION=on
    SEQ_INPUT_MODE=pep_condition
  ...
  PCS_RC_LC_R1_FF_R3_RESIDUE_CFM|F03_FF_R3_RESIDUE_CFM)
    ABLATION_PARENT=PCS_RC_LC_R1_FF_R3_RESIDUE_ENDPOINT
    EXPERIMENT_FACTOR=foldflow_r3_constant_conditional_velocity
    MODULE_ID=F03_FF_R3_RESIDUE_CFM
    MODULE_PARENT=F02_FF_R3_RESIDUE_ENDPOINT
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=foldflow_r3_residue_cfm
    T_SAMPLING=uniform
    SAMPLER_MODE=bridge
    COORD_PEP_AS_CONDITION=on
    SEQ_INPUT_MODE=pep_condition
  ...
  *)
    ...
    echo "  PCS_RC_LC_R1_FF_R3_RESIDUE_ENDPOINT"
    echo "  PCS_RC_LC_R1_FF_R3_RESIDUE_CFM"
    exit 2
    ;;
esac
...
if [[ "$MODE" != "train" ]]; then
  ...
  echo "FoldFlow-R3 EXP_ID: PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT, PCS_RC_LC_R1_FF_R3_RESIDUE_ENDPOINT, PCS_RC_LC_R1_FF_R3_RESIDUE_CFM"
  exit 2
fi
