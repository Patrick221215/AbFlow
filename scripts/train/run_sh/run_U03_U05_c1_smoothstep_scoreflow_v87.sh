...
case "$EXP_ID" in
  ...
  U05_PCS_RC_LC_R1_FF_R3_C1_SMOOTHSTEP_CANONICAL_CARRIER)
    ABLATION_PARENT=U03_PCS_RC_LC_R1_FF_R3_BOUNDARY_REGULAR_CARRIER
    EXPERIMENT_FACTOR=c1_smoothstep_source_anchored_canonical_chart
    MODULE_ID=U05_F01_C1_SMOOTHSTEP_CANONICAL_CARRIER
    MODULE_PARENT=U03_F01_BOUNDARY_REGULAR_CARRIER
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=f01_r3_c1_smoothstep_canonical_carrier
    T_SAMPLING=uniform
    SAMPLER_MODE=f01_c1_smoothstep_canonical_carrier
    COORD_PEP_AS_CONDITION=on
    SEQ_INPUT_MODE=pep_condition
    SHADOW_SEQ_STATE=off
    DUAL_SEQUENCE_STATE=off
  ...
  *)
    ...
    echo "Supported v87 experiments:"
    echo "  U03_PCS_RC_LC_R1_FF_R3_BOUNDARY_REGULAR_CARRIER"
    echo "  U05_PCS_RC_LC_R1_FF_R3_C1_SMOOTHSTEP_CANONICAL_CARRIER"
    exit 2
    ;;
esac
