...
# ============================================================
# AbFlow v84 experiment definitions on completed historical F01:
#   G00 = AbX-inspired interface pair-time
#   G01 = Endpoint-implied global Flow pair semantics
#   G02 = G01 + stable variance-scaled Score / Score-Flow agreement
#
# All three freeze:
# PCS-RC + recurrent proposal context + adaptive-g FF-R3 global H3 path
# + clean Endpoint loss + bridge sampler + integrated_endpoint.
# No independent Score head, DSM/pathflow/SF2M loss, SATC, or new noise path.
# ============================================================
STATE_PATH=on
PER_SAMPLE_T=on
TIME_EMBED=on
...
case "$EXP_ID" in
  G00_PCS_RC_LC_R1_FF_R3_INTERFACE_PAIR_TIME)
    ABLATION_PARENT=PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT
    EXPERIMENT_FACTOR=abx_inspired_interface_pair_time_semantics
    MODULE_ID=G00_FF_R3_INTERFACE_PAIR_TIME
  ...
  G01_PCS_RC_LC_R1_FF_R3_FLOW_PAIR_CONTROL)
    ABLATION_PARENT=PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT
    EXPERIMENT_FACTOR=endpoint_implied_global_flow_pair_semantics
    MODULE_ID=G01_FF_R3_FLOW_PAIR_CONTROL
    MODULE_PARENT=F01_FF_R3_GLOBAL_ENDPOINT
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
  ...
  G02_PCS_RC_LC_R1_FF_R3_STABLE_SCOREFLOW_PAIR_COUPLED)
    ABLATION_PARENT=G01_PCS_RC_LC_R1_FF_R3_FLOW_PAIR_CONTROL
    EXPERIMENT_FACTOR=add_stable_score_residual_and_scoreflow_agreement
    MODULE_ID=G02_FF_R3_STABLE_SCOREFLOW_PAIR_COUPLED
    MODULE_PARENT=G01_FF_R3_FLOW_PAIR_CONTROL
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=foldflow_r3_global_endpoint
  ...
  *)
    ...
    echo "Supported F01 child experiments:"
    echo "  G00_PCS_RC_LC_R1_FF_R3_INTERFACE_PAIR_TIME"
    echo "  G01_PCS_RC_LC_R1_FF_R3_FLOW_PAIR_CONTROL"
    echo "  G02_PCS_RC_LC_R1_FF_R3_STABLE_SCOREFLOW_PAIR_COUPLED"
    exit 2
    ;;
esac
...
if [[ "$MODE" != "train" ]]; then
  ...
  echo "EXP_ID: G00_PCS_RC_LC_R1_FF_R3_INTERFACE_PAIR_TIME, G01_PCS_RC_LC_R1_FF_R3_FLOW_PAIR_CONTROL, G02_PCS_RC_LC_R1_FF_R3_STABLE_SCOREFLOW_PAIR_COUPLED"
  exit 2
fi
...
runtime = {
    ...
    "pair_time_adapter": ("abx_style_sinusoidal_semantic_residual" if os.environ.get("ABFLOW_PAIR_TIME_SCOPE", "off") != "off" else "off"),
    "scoreflow_pair_adapter": ("zero_start_parameter_matched_5_channel_projection" if os.environ.get("ABFLOW_SCOREFLOW_PAIR_MODE", "off") != "off" else "off"),
    "scoreflow_pair_mode": os.environ.get("ABFLOW_SCOREFLOW_PAIR_MODE", "off"),
