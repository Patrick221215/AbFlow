...
# ============================================================
# AbFlow v60: F01-centered global-R3 mechanism convergence stage
# ============================================================
# Existing anchor (do not rerun if already active/completed):
#   F01 GLOBAL_ENDPOINT
#       FoldFlow-R3 sqrt(t(1-t)) global H3 translation state
#       + clean native endpoint target.
#
# New formal children:
#   F04 GLOBAL_CFM
#       Same stochastic states as F01; ONLY target semantics change to
#       FoldFlow Euclidean u*=X1-X0 through endpoint parameterization.
#
#   F05 GLOBAL_ENDPOINT_PAIR_TIME_INTERFACE
#       Same path and target as F01; ONLY true Ab-Ag interface/surface
#       messages receive explicit pair-level flow-time modulation.
#
# Historical F02/F03 residue-R3 runs are retained as mechanism evidence but
# are intentionally paused: residue-independent R3 mixes global placement
# and local deformation and can disrupt inter-residue peptide geometry.
# ============================================================
STATE_PATH=on
PER_SAMPLE_T=on
TIME_EMBED=on
...
case "$EXP_ID" in
  PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT|F01_FF_R3_GLOBAL_ENDPOINT)
    # Existing anchor. Keep for checkpoint test compatibility / optional resume.
    ABLATION_PARENT=PCS_RC_LC_R1_STRUCT_GLOBAL_ENDPOINT
    EXPERIMENT_FACTOR=foldflow_r3_sqrt_schedule_global_state
    MODULE_ID=F01_FF_R3_GLOBAL_ENDPOINT
  ...
  PCS_RC_LC_R1_FF_R3_GLOBAL_CFM|F04_FF_R3_GLOBAL_CFM)
    # F01 single-factor child:
    # SAME global-R3 stochastic states; ONLY target X1 -> FoldFlow u*=X1-X0.
    ABLATION_PARENT=PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT
    EXPERIMENT_FACTOR=foldflow_global_constant_conditional_velocity
    MODULE_ID=F04_FF_R3_GLOBAL_CFM
    MODULE_PARENT=F01_FF_R3_GLOBAL_ENDPOINT
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=foldflow_r3_global_cfm
    T_SAMPLING=uniform
    SAMPLER_MODE=bridge
    COORD_PEP_AS_CONDITION=on
    SEQ_INPUT_MODE=pep_condition
  ...
  PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT_PAIR_TIME_INTERFACE|F05_FF_R3_GLOBAL_ENDPOINT_PAIR_TIME_INTERFACE)
    # F01 single-factor child:
    # SAME global-R3 stochastic path and clean endpoint target; ONLY the
    # true antibody-antigen local/surface messages become explicitly time-aware.
    ABLATION_PARENT=PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT
    EXPERIMENT_FACTOR=foldflow_interface_pair_time_representation
    MODULE_ID=F05_FF_R3_GLOBAL_ENDPOINT_PAIR_TIME_INTERFACE
    MODULE_PARENT=F01_FF_R3_GLOBAL_ENDPOINT
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
  ...
  *)
    ...
    echo "Supported formal v60 profiles:"
    echo "  PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT"
    echo "  PCS_RC_LC_R1_FF_R3_GLOBAL_CFM"
    echo "  PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT_PAIR_TIME_INTERFACE"
    exit 2
    ;;
esac
...
if [[ "$MODE" != "train" ]]; then
  ...
  echo "FoldFlow-R3 v60 EXP_ID: PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT, PCS_RC_LC_R1_FF_R3_GLOBAL_CFM, PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT_PAIR_TIME_INTERFACE"
  exit 2
fi
