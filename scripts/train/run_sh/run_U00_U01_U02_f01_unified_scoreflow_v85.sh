...
# ============================================================
# AbFlow v85: F01-anchored unified Score--Flow
# ============================================================
# All three configs freeze the completed historical F01 scientific parent:
...
#
# U00: AbX-style explicit residue-pair time BEFORE the native edge MLP.
# U01: single-field canonical Score--Flow carrier with only a small t<0.05
#      mathematical Endpoint boundary.
# U02: same carrier, but clean Endpoint target for t<0.20 (pre-registered
#      historical DSM boundary) and canonical Score--Flow target thereafter.
#
# U01/U02 add NO score head, NO flow head, and NO lambda_score/lambda_flow.
# The same coordinate output is the only dynamic carrier.
# ============================================================
STATE_PATH=on
PER_SAMPLE_T=on
TIME_EMBED=on
...
case "$EXP_ID" in
  U00_PCS_RC_LC_R1_FF_R3_ABX_PAIR_TIME_EDGE)
    ABLATION_PARENT=PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT
    EXPERIMENT_FACTOR=abx_style_explicit_pre_edge_pair_time
    MODULE_ID=U00_F01_ABX_PAIR_TIME_EDGE
  ...
  U01_PCS_RC_LC_R1_FF_R3_CANONICAL_SCORE_FLOW_CARRIER)
    ABLATION_PARENT=PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT
    EXPERIMENT_FACTOR=f01_adaptive_g_single_field_canonical_scoreflow_carrier
    MODULE_ID=U01_F01_CANONICAL_SCOREFLOW_CARRIER
    MODULE_PARENT=F01_FF_R3_GLOBAL_ENDPOINT
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=f01_r3_canonical_carrier
    T_SAMPLING=uniform
  ...
  U02_PCS_RC_LC_R1_FF_R3_ENDPOINT_CANONICAL_HYBRID)
    ABLATION_PARENT=PCS_RC_LC_R1_FF_R3_GLOBAL_ENDPOINT
    EXPERIMENT_FACTOR=endpoint_boundary_plus_f01_canonical_scoreflow_interior
    MODULE_ID=U02_F01_ENDPOINT_CANONICAL_HYBRID
    MODULE_PARENT=F01_FF_R3_GLOBAL_ENDPOINT
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
  ...
  *)
    ...
    echo "Supported v85 F01-unified experiments:"
    echo "  U00_PCS_RC_LC_R1_FF_R3_ABX_PAIR_TIME_EDGE"
    echo "  U01_PCS_RC_LC_R1_FF_R3_CANONICAL_SCORE_FLOW_CARRIER"
    echo "  U02_PCS_RC_LC_R1_FF_R3_ENDPOINT_CANONICAL_HYBRID"
    exit 2
    ;;
esac
