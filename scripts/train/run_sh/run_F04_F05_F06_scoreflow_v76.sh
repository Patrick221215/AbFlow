...
# ============================================================
# AbFlow v76: fixed source-noise normalized Score--Flow coupling stage
# ============================================================
# Three endpoint-free experiments (config stem = EXP_ID = run folder):
#   F04_PCS_RC_LC_R1_CLEAN_MEAN_FLOW
#       clean deterministic state and direct mean velocity X1-X0.
#   F05_PCS_RC_LC_R1_FIXEDG_CANONICAL_SCORE_FLOW
#       fixed source-noise canonical probability Flow in one carrier.
#   F06_PCS_RC_LC_R1_FLOW_CONDITIONED_SCORE_FLOW
#       explicit q=sigma*score coupled to clean mean Flow.
#
# F04 is the zero-Score control. F05/F06 share one source/target-independent
# Gaussian width, so the stochastic path is known during both training and
# inference. F06 reconstructs u=m-sigma_dot*q in training and sampling.
# Endpoint loss, terminal projection, SATC, pair-time, SO(3), ESM2 and OT stay off.
# ============================================================
STATE_PATH=on
PER_SAMPLE_T=on
TIME_EMBED=on
...
case "$EXP_ID" in
  F04_PCS_RC_LC_R1_CLEAN_MEAN_FLOW)
    ...
    EXPERIMENT_FACTOR=remove_stochastic_state_target_mismatch
    MODULE_ID=F04_CLEAN_MEAN_FLOW
    MODULE_PARENT=F02_MEAN_FLOW_CONTROL
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    ...
    FINAL_READOUT_MODE=integrated_state
  ...
  F05_PCS_RC_LC_R1_FIXEDG_CANONICAL_SCORE_FLOW)
    ABLATION_PARENT=F04_PCS_RC_LC_R1_CLEAN_MEAN_FLOW
    EXPERIMENT_FACTOR=fixed_inference_known_gaussian_canonical_probability_flow
    MODULE_ID=F05_FIXEDG_CANONICAL_SCORE_FLOW
    MODULE_PARENT=F04_CLEAN_MEAN_FLOW
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=fixedg_r3_canonical_score_flow
    T_SAMPLING=sf2m_uniform
    SAMPLER_MODE=sf2m_ode
    COORD_PEP_AS_CONDITION=on
    SEQ_INPUT_MODE=pep_condition
  ...
  F06_PCS_RC_LC_R1_FLOW_CONDITIONED_SCORE_FLOW)
    ABLATION_PARENT=F05_PCS_RC_LC_R1_FIXEDG_CANONICAL_SCORE_FLOW
    EXPERIMENT_FACTOR=explicit_flow_conditioned_scaled_score_factorization
    MODULE_ID=F06_FLOW_CONDITIONED_SCORE_FLOW
    MODULE_PARENT=F05_FIXEDG_CANONICAL_SCORE_FLOW
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=fixedg_r3_flow_conditioned_score_flow
    T_SAMPLING=sf2m_uniform
    SAMPLER_MODE=fixedg_scaled_score_ode
    COORD_PEP_AS_CONDITION=on
    SEQ_INPUT_MODE=pep_condition
    SHADOW_SEQ_STATE=off
    DUAL_SEQUENCE_STATE=off
  ...
  *)
    ...
    echo "Supported v76 experiments:"
    echo "  F04_PCS_RC_LC_R1_CLEAN_MEAN_FLOW"
    echo "  F05_PCS_RC_LC_R1_FIXEDG_CANONICAL_SCORE_FLOW"
    echo "  F06_PCS_RC_LC_R1_FLOW_CONDITIONED_SCORE_FLOW"
    exit 2
    ;;
esac
...
#     bash scripts/train/run_F04_F05_F06_scoreflow_v76.sh train <EXP_ID> <GPU_ID> <BASE_CONFIG>
...
if [[ "$MODE" != "train" ]]; then
  ...
  echo "v76 EXP_ID: F04_PCS_RC_LC_R1_CLEAN_MEAN_FLOW, F05_PCS_RC_LC_R1_FIXEDG_CANONICAL_SCORE_FLOW, F06_PCS_RC_LC_R1_FLOW_CONDITIONED_SCORE_FLOW"
  exit 2
fi
