...
SUPPORTED_EXP="U07_PCS_RC_LC_R1_FF_R3_STRUCTURE_CONDITIONED_SEQUENCE"
if [[ "$EXP_ID" != "$SUPPORTED_EXP" ]]; then
  echo "Unknown EXP_ID: $EXP_ID"
  echo "Supported: $SUPPORTED_EXP"
  exit 2
fi

# ------------------------------------------------------------------
# U07 scientific semantics: exact U02 path/target/sampler + one readout change
# ------------------------------------------------------------------
export ABFLOW_ABLATION_PARENT="U02_PCS_RC_LC_R1_FF_R3_ENDPOINT_CANONICAL_HYBRID"
export ABFLOW_EXPERIMENT_FACTOR="prediction_derived_structure_to_sequence_interface_readout"
export ABFLOW_SINGLE_FACTOR_ABLATION="true"
export ABFLOW_MODULE_ID="U07_F01_STRUCTURE_CONDITIONED_SEQUENCE"
export ABFLOW_MODULE_PARENT="U02_F01_ENDPOINT_CANONICAL_HYBRID"
...
# U07 only scientific factor: preserve original static geometry; strengthen structure -> sequence readout.
export ABFLOW_COORDINATE_AUTHORITY="legacy_dual"
export ABFLOW_STRUCTURE_SEQ_READOUT="interface_geometry"
...
# ------------------------------------------------------------------
# attach watcher to an already-running U07
# ------------------------------------------------------------------
...
if [[ "$MODE" != "train" ]]; then
  echo "Train:       bash $0 train U07_PCS_RC_LC_R1_FF_R3_STRUCTURE_CONDITIONED_SEQUENCE 6,7 <config.json>"
  echo "Test bridge: bash $0 test  U07_PCS_RC_LC_R1_FF_R3_STRUCTURE_CONDITIONED_SEQUENCE 0 <ckpt> <result_dir> [test.json]"
  echo "Attach eval: bash $0 attach_eval U07_PCS_RC_LC_R1_FF_R3_STRUCTURE_CONDITIONED_SEQUENCE 0 [eval_gpu|auto]"
  exit 2
fi
...
if [[ "$RUN_MODE" == "resume" ]]; then
  ...
  # Do not silently mix a fresh run into an existing U07 directory.
