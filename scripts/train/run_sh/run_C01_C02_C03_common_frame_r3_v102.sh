...
# ============================================================
# AbFlow v102: Common Conditioning Frame + R3 Support Geometry
# ============================================================
# Scientific parent: U02 endpoint/canonical hybrid.
#
# C01: ONLY replace the historical split Ag/Ab translation gauges by one
#      inference-available framework-defined common complex gauge.
# C02: C01 + change stochastic support from graph-rigid global R3 to
#      residue-rigid R3, while keeping U02's hybrid canonical target.
# C03: C02 + remove the graph-wise mean from the residue stochastic residual,
#      separating deterministic global placement from stochastic loop shape.
#
# No auxiliary score/flow head, Pair-Time, SATC, support-factorized loss,
# recurrent translation-credit loss, or gradient surgery is used.
# ============================================================

# ---------- frozen U02 scientific parent ----------
...
# Explicitly freeze previously-tested branches OFF.
...
# Keep current structure/sequence semantics identical to U02.
...
# All formal v102 experiments use the same target-leakage-free common gauge.
export ABFLOW_COORD_FRAME_MODE="common_framework"

case "$EXP_ID" in
  C01_PCS_RC_LC_R1_FF_R3_U02_COMMON_FRAME_GLOBAL_R3)
    export ABFLOW_ABLATION_PARENT="U02_PCS_RC_LC_R1_FF_R3_ENDPOINT_CANONICAL_HYBRID"
    export ABFLOW_EXPERIMENT_FACTOR="single_common_framework_coordinate_gauge"
    export ABFLOW_SINGLE_FACTOR_ABLATION="true"
    export ABFLOW_MODULE_ID="C01_COMMON_FRAME_GLOBAL_R3"
  ...
  C02_PCS_RC_LC_R1_FF_R3_COMMON_FRAME_RESIDUE_R3_SCOREFLOW)
    export ABFLOW_ABLATION_PARENT="C01_PCS_RC_LC_R1_FF_R3_U02_COMMON_FRAME_GLOBAL_R3"
    export ABFLOW_EXPERIMENT_FACTOR="global_r3_to_residue_rigid_r3_support"
    export ABFLOW_SINGLE_FACTOR_ABLATION="true"
    export ABFLOW_MODULE_ID="C02_COMMON_FRAME_RESIDUE_R3"
    export ABFLOW_MODULE_PARENT="C01_COMMON_FRAME_GLOBAL_R3"

    export ABFLOW_SCOREFM_LOSS_MODE="f01_r3_residue_endpoint_canonical_hybrid"
    export ABFLOW_SCOREFM_SAMPLER_MODE="f01_residue_canonical_carrier"
    ;;

  C03_PCS_RC_LC_R1_FF_R3_COMMON_FRAME_CENTERED_RESIDUE_R3_SCOREFLOW)
    export ABFLOW_ABLATION_PARENT="C02_PCS_RC_LC_R1_FF_R3_COMMON_FRAME_RESIDUE_R3_SCOREFLOW"
    export ABFLOW_EXPERIMENT_FACTOR="center_only_residue_stochastic_residual"
    export ABFLOW_SINGLE_FACTOR_ABLATION="true"
    export ABFLOW_MODULE_ID="C03_COMMON_FRAME_CENTERED_RESIDUE_R3"
    export ABFLOW_MODULE_PARENT="C02_COMMON_FRAME_RESIDUE_R3"

    export ABFLOW_SCOREFM_LOSS_MODE="f01_r3_centered_residue_endpoint_canonical_hybrid"
    export ABFLOW_SCOREFM_SAMPLER_MODE="f01_centered_residue_canonical_carrier"
    ;;

  *)
    echo "Unknown EXP_ID: $EXP_ID"
    echo "Supported v102:"
    echo "  C01_PCS_RC_LC_R1_FF_R3_U02_COMMON_FRAME_GLOBAL_R3"
    echo "  C02_PCS_RC_LC_R1_FF_R3_COMMON_FRAME_RESIDUE_R3_SCOREFLOW"
    echo "  C03_PCS_RC_LC_R1_FF_R3_COMMON_FRAME_CENTERED_RESIDUE_R3_SCOREFLOW"
    exit 2
    ;;
esac
...
export ABFLOW_DIAGNOSTIC_FILE_INTERVAL="${ABFLOW_DIAGNOSTIC_FILE_INTERVAL:-0}"
...
print_settings() {
  ...
  echo "COORD_FRAME_MODE=$ABFLOW_COORD_FRAME_MODE"
...
if [[ "$MODE" == "test" ]]; then
  ...
  # C01 and C02 intentionally share evaluation GPU 0.  Serialize actual
  # test jobs per physical GPU so two independent AutoTopK watchers cannot
  # collide/OOM when checkpoints appear at the same time.
  EVAL_LOCK="${ABFLOW_EVAL_LOCK_DIR:-/tmp}/abflow_eval_gpu_${GPU_ID}.lock"
  if command -v flock >/dev/null 2>&1; then
    mkdir -p "$(dirname "$EVAL_LOCK")"
    exec 9>"$EVAL_LOCK"
    echo "[AutoTopK] waiting for eval GPU lock: $EVAL_LOCK"
    flock 9
    echo "[AutoTopK] acquired eval GPU lock: $EVAL_LOCK"
  fi
...
# ------------------------------------------------------------------
# attach watcher to an already-running v102 experiment
# ------------------------------------------------------------------
...
if [[ "$MODE" != "train" ]]; then
  echo "Train:       bash $0 train <C01|C02|C03 EXP_ID> 2,3 <config.json>"
  echo "Test bridge: bash $0 test  <C01|C02|C03 EXP_ID> 0 <ckpt> <result_dir> [test.json]"
  echo "Attach eval: bash $0 attach_eval <C01|C02|C03 EXP_ID> 0 [eval_gpu|auto]"
  exit 2
fi
...
runtime = {
    ...
    "coord_frame_mode": os.environ.get("ABFLOW_COORD_FRAME_MODE", ""),
