...
# ============================================================
# AbFlow Stage-2: FoldFlow-informed full-atom causal ladder
# ============================================================
# Scientific rule:
...
case "$EXP_ID" in
  PCS_RC_LC_R1_GT_PAIR_TIME_INTERFACE|P01_GT_PAIR_TIME_INTERFACE)
    # P01: existing GT-SATC + interface-only pair-time.
    # Common parent: PCS_RC_LC_R1_GT_SATC.
    # Factor setting: interface_time=1, context_time=0.
    ABLATION_PARENT=PCS_RC_LC_R1_GT_SATC
    EXPERIMENT_FACTOR=interface_pair_level_time_conditioning
    MODULE_ID=P01_GT_PAIR_TIME_INTERFACE
    MODULE_PARENT=M02_GT_SATC
  ...
  PCS_RC_LC_R1_GT_PAIR_TIME_CONTEXT|P02_GT_PAIR_TIME_CONTEXT)
    # P02: existing GT-SATC + context-only pair-time.
    # Common parent: PCS_RC_LC_R1_GT_SATC.
    # Factor setting: interface_time=0, context_time=1.
    # True Ab-Ag local cross edges and surface messages are unchanged.
    ABLATION_PARENT=PCS_RC_LC_R1_GT_SATC
    EXPERIMENT_FACTOR=context_pair_level_time_conditioning
    MODULE_ID=P02_GT_PAIR_TIME_CONTEXT
    MODULE_PARENT=M02_GT_SATC
  ...
  PCS_RC_LC_R1_GT_PAIR_TIME_ALL|P03_GT_PAIR_TIME_ALL)
    # P03: existing GT-SATC + interface-time + context-time.
    # Common parent for factorial interpretation remains the existing GT-SATC.
    # Factor setting: interface_time=1, context_time=1.
    ABLATION_PARENT=PCS_RC_LC_R1_GT_SATC
    EXPERIMENT_FACTOR=all_pair_level_time_conditioning
    MODULE_ID=P03_GT_PAIR_TIME_ALL
    MODULE_PARENT=M02_GT_SATC
  ...
  *)
    ...
    echo "Supported new full-run profiles:"
    echo "  PCS_RC_LC_R1_GT_PAIR_TIME_INTERFACE"
    echo "  PCS_RC_LC_R1_GT_PAIR_TIME_CONTEXT"
    echo "  PCS_RC_LC_R1_GT_PAIR_TIME_ALL"
    exit 2
    ;;
esac
...
if [[ "$MODE" != "train" ]]; then
  ...
  echo "Stage-2 EXP_ID: PCS_RC_LC_R1_GT_PAIR_TIME_INTERFACE, PCS_RC_LC_R1_GT_PAIR_TIME_CONTEXT, PCS_RC_LC_R1_GT_PAIR_TIME_ALL"
  exit 2
fi
