...
# ============================================================
# AbFlow v74: F02 clean mean-flow optimization stage
# ============================================================
...
#   F05_PCS_RC_LC_R1_FACTORIZED_MEAN_FLOW
#       F04 + explicit global-placement/local-conformation velocity reduction.
#   F06_PCS_RC_LC_R1_FACTORIZED_PTC_MEAN_FLOW
#       F05 + paired-time straight-path velocity consistency.
#
# Every configuration keeps the F02 theoretical object: a direct full-atom
# conditional mean velocity integrated by the same Euler ODE. Endpoint loss,
# terminal projection, stochastic score correction, independent score heads,
# SATC, pair-time representation, SO(3), ESM2 and OT are all disabled.
# ============================================================
STATE_PATH=on
PER_SAMPLE_T=on
TIME_EMBED=on
...
case "$EXP_ID" in
  ...
  F05_PCS_RC_LC_R1_FACTORIZED_MEAN_FLOW)
    ...
    EXPERIMENT_FACTOR=factorize_global_placement_and_local_conformation_velocity
    MODULE_ID=F05_FACTORIZED_MEAN_FLOW
    MODULE_PARENT=F04_CLEAN_MEAN_FLOW
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=clean_r3_factorized_mean_flow
    T_SAMPLING=sf2m_uniform
    SAMPLER_MODE=sf2m_ode
    COORD_PEP_AS_CONDITION=on
    SEQ_INPUT_MODE=pep_condition
  ...
  F06_PCS_RC_LC_R1_FACTORIZED_PTC_MEAN_FLOW)
    ABLATION_PARENT=F05_PCS_RC_LC_R1_FACTORIZED_MEAN_FLOW
    EXPERIMENT_FACTOR=paired_time_straight_path_velocity_consistency
    MODULE_ID=F06_FACTORIZED_PTC_MEAN_FLOW
    MODULE_PARENT=F05_FACTORIZED_MEAN_FLOW
    SINGLE_FACTOR_ABLATION=true
    SOURCE_MODE=pcs_rc
    RECURRENT_PROPOSAL_CONTEXT=on
    LOSS_MODE=clean_r3_factorized_ptc_mean_flow
    T_SAMPLING=sf2m_uniform
    SAMPLER_MODE=sf2m_ode
    COORD_PEP_AS_CONDITION=on
    SEQ_INPUT_MODE=pep_condition
  ...
  *)
    ...
    echo "Supported v74 experiments:"
    echo "  F04_PCS_RC_LC_R1_CLEAN_MEAN_FLOW"
    echo "  F05_PCS_RC_LC_R1_FACTORIZED_MEAN_FLOW"
    echo "  F06_PCS_RC_LC_R1_FACTORIZED_PTC_MEAN_FLOW"
    exit 2
    ;;
esac
...
#     bash scripts/train/run_F02_meanflow_v74.sh train <EXP_ID> <GPU_ID> <BASE_CONFIG>
...
if [[ "$MODE" != "train" ]]; then
  ...
  echo "v74 EXP_ID: F04_PCS_RC_LC_R1_CLEAN_MEAN_FLOW, F05_PCS_RC_LC_R1_FACTORIZED_MEAN_FLOW, F06_PCS_RC_LC_R1_FACTORIZED_PTC_MEAN_FLOW"
  exit 2
fi
