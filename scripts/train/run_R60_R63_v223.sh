...
usage() {
  cat >&2 <<'USAGE'
Usage:
  bash scripts/train/run_R60_R63_v223_1.sh <config.json> \
...
if 'gpus' in runtime:
    raise SystemExit(
        "runtime.gpus is forbidden in V223.1 configs; pass physical GPUs with --gpus"
    )
output = training['output_dir']
if not os.path.isabs(output):
    output = os.path.abspath(os.path.join(root, output))
# Backward-compatible fallback only. Formal V223.1 configs leave this empty.
...
# V223.1 task-localized relational diagnostics.  These never alter losses, optimizer, sampler,
...
# V223 formal Test failure contract. Infrastructure/protocol failures must abort;
...
if not exp_id:
    raise SystemExit('V223.1 identity preflight failed: experiment.id is missing')
if not (exp_id == config_stem == output_id):
    raise SystemExit(
        'V223.1 identity preflight failed:\n'
...
if generation_dir:
    ...
    if generation_id != exp_id:
        raise SystemExit(
            'V223 generation provenance preflight failed:\n'
            f'  experiment.id={exp_id}\n'
...
if protocol != 'formal_train_val_test':
    raise SystemExit(
        f"V223.1 requires experiment.protocol='formal_train_val_test'; got {protocol!r}"
    )
test_set = str(cfg.get('data', {}).get('test', {}).get('set', '') or '').strip()
if os.path.basename(test_set) != 'test.json':
    raise SystemExit(
        f"V223.1 requires data.test.set to remain observational test.json; got {test_set!r}"
    )
...
if ! grep -q "GradientNormOverflowRecovered" "$PROJECT_ROOT/trainer/abs_trainer.py"; then
  echo "V223.1 requires the already-installed V10 stable finite-gradient norm fallback in trainer/abs_trainer.py" >&2
  exit 2
fi
...
for rel in [
    ...
    'train.py',
    'scripts/train/run_R60_R63_v223_1.sh',
]:
...
if state_mode != 'scoreflow_endpoint_fused':
    raise SystemExit(
        "V223.1 requires coordinate_state.mode='scoreflow_endpoint_fused'; "
        f"got {state_mode!r}"
    )
...
if pc_mode != 'direct_shared':
    raise SystemExit(
        f"V223.1 requires pair_coordinate.mode='direct_shared'; got {pc_mode!r}"
    )
if 'delta_bound' in pc:
    raise SystemExit(
        'V223.1 direct_shared must not carry stale pair_coordinate.delta_bound; '
...
if cc_mode != 'egnn_prenorm_raw':
    raise SystemExit(
        'V223.1 requires coordinate_controller.mode=egnn_prenorm_raw; '
        f'got {cc_mode!r}'
    )
...
if disto_weight > 0.0 and disto_scope != 'generation_anchored':
    raise SystemExit(
        'V223.1 trainable Distogram must use pair_scope=generation_anchored so every '
...
allowed={'pred_design_endpoint','carrier_implied_endpoint','terminal_fused_endpoint'}
if sl_source not in allowed:
    raise SystemExit(f'V223.1 invalid smooth-lDDT prediction_source={sl_source!r}; allowed={sorted(allowed)}')
...
if sl_source == 'carrier_implied_endpoint':
    sl_alignment = 'exact_carrier_endpoint'
elif sl_source == 'terminal_fused_endpoint':
    sl_alignment = 'state_closed_terminal_fused_endpoint'
else:
    sl_alignment = 'legacy_full_pred_X'
print(
    '[ScoreFlowStateContract] '
    f'coordinate_state={state_mode} endpoint_authority=single_fused_h3 '
...
print(
    ...
    f'fixed_context=restored sampler_alignment={sl_alignment}'
)
PY
