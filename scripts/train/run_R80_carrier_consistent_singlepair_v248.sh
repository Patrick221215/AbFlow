...
# R80 single-factor launcher: clean R79 + carrier-consistent round-state Single/Pair.
# Scientific delta: rounds1/2 relational geometry uses previous carrier instead
# of previous analytic endpoint. Train->Validation->Test, F01/loss/sampler,
# leakage barrier and single physical Cartesian authority remain unchanged.
...
usage() {
  cat >&2 <<'USAGE'
Usage:
  bash scripts/train/run_R80_carrier_consistent_singlepair_v248.sh <R80-config.json> \
    --gpus 2,3,4,5,6,7 --port 29782

R80 contract:
  - parent = leakage-safe R79 and starts from scratch;
  - round0 relational H3 state = outer Xt; rounds1/2 = previous carrier;
  - relational Single/Pair and EGNN Cartesian actuator read the same carrier state;
  - no prev_seq/prev_pair/prev_pos, no new loss/sampler/controller/Cartesian field;
  - H3 native xloss_mask remains blocked; Pair remains static inside each round;
...
if role!='r80_carrier_consistent_singlepair_only': raise SystemExit(f'R80 role required, got {role!r}')
if exp.get('parent')!='R79_R05_ABX_R77_ROUND_STATE_SINGLEPAIR_ANALYTIC3R_TVT_U02': raise SystemExit('R80 must use clean R79 as parent identity')
if str(exp.get('initialization','')).lower()!='scratch': raise SystemExit('R80 must start from scratch')
if str(cfg['training']['schedule'].get('resume_checkpoint','') or '').strip(): raise SystemExit('R80 forbids resume_checkpoint')
if int(cfg['model']['architecture']['iter_round']) != 3: raise SystemExit('R80 requires exactly 3 refinement rounds')
...
if rs.get('round0_state')!='outer_Xt' or rs.get('later_round_state')!='previous_carrier': raise SystemExit('R80 requires outer_Xt -> previous_carrier relational recurrence')
...
if bool(rs.get('detach_between_rounds',True)): raise SystemExit('R80 keeps differentiable physical recurrence')
...
ex=sp.get('execution',{})
if int(ex.get('triangle_chunk_size',64)) != 64 or int(ex.get('pair_distance_chunk_size',8)) != 8: raise SystemExit('R80 restores matched train chunks 64/8')
if int(ex.get('triangle_chunk_size_eval',64)) != 64 or int(ex.get('pair_distance_chunk_size_eval',8)) != 8: raise SystemExit('R80 restores matched eval chunks 64/8')
if not bool(ex.get('static_layout_cache',False)): raise SystemExit('R80 requires execution-only static layout reuse')
...
print('[RoundStateSinglePairContract] refresh=once_per_macro_round round0=outer_Xt later=previous_carrier representation_state=carrier actuator_state=carrier pair_static_within_round=1 prev_seq=0 prev_pair=0 prev_pos=0 detach=0')
...
print('[ScientificDeltaContract] versus_clean_R79=relational_later_state_endpoint_to_carrier_only new_loss=0 new_sampler=0 new_controller=0 new_cartesian_field=0 prev_recycle=0')
print('[DiagnosticsContract] RoundTransportValidation=on stale_refresh_metrics=off TestFieldTrajectory=compact StateExposureAudit=off')
PY2
...
grep -Fq 'self.round_state_later_state == "previous_carrier"' "$PROJECT_ROOT/models/AbFlow/AbFlow_model.py" || { echo 'carrier-consistent relational recurrence missing' >&2; exit 2; }
grep -Fq 'current_relational_h3_native = self._interface_to_native_model(' "$PROJECT_ROOT/models/AbFlow/AbFlow_model.py" || { echo 'carrier -> native relational conversion missing' >&2; exit 2; }
...
echo '[Preflight] py_compile=PASS carrier_consistent=PASS h3_native_observation_blocked=PASS static_layout=PASS validation_bridge_capture=OFF train_val_test=UNCHANGED epoch_test=ON prev_recycle=OFF single_field=PASS final_round_only=PASS sampler_unchanged=PASS'

python - "$PROJECT_ROOT" <<'PY2'
import hashlib,os,sys
root=sys.argv[1]
for rel in ['models/AbFlow/AbFlow_model.py','models/AbFlow/abflow_components.py','utils/nn_utils.py','models/modules/am_enc.py','models/modules/am_egnn.py','trainer/AbFlow_trainer.py','train.py','scripts/train/run_R80_carrier_consistent_singlepair_v248.sh']:
