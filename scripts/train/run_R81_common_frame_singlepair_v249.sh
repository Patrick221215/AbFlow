...
usage() {
  cat >&2 <<'USAGE'
Usage:
  bash scripts/train/run_R81_common_frame_singlepair_v249.sh <R80-config.json> \
...
print('[ScientificDeltaContract] versus_clean_R79=cross_chain_pair_geometry_common_complex_frame_only new_loss=0 new_sampler=0 new_controller=0 new_cartesian_field=0 prev_recycle=0')
print('[DiagnosticsContract] RoundTransportValidation=compact PairFrameValidation=epoch0 TestFieldTrajectory=compact stale_refresh_metrics=off StateExposureAudit=off')
PY2
...
root=sys.argv[1]
for rel in ['models/AbFlow/AbFlow_model.py','models/AbFlow/abflow_components.py','utils/nn_utils.py','models/modules/am_enc.py','models/modules/am_egnn.py','trainer/AbFlow_trainer.py','train.py','scripts/train/run_R81_common_frame_singlepair_v249.sh']:
