...
usage() {
  cat >&2 <<'USAGE'
Usage:
  bash scripts/train/run_R41_R43_v216.sh <config.json> \
...
if 'gpus' in runtime:
    raise SystemExit(
        "runtime.gpus is forbidden in V216 configs; pass physical GPUs with --gpus"
    )
output = training['output_dir']
if not os.path.isabs(output):
    output = os.path.abspath(os.path.join(root, output))
# Backward-compatible fallback only. Formal V216 configs leave this empty.
...
if not exp_id:
    raise SystemExit('V216 identity preflight failed: experiment.id is missing')
if not (exp_id == config_stem == output_id):
    raise SystemExit(
        'V216 identity preflight failed:\n'
...
if generation_dir:
    ...
    if generation_id != exp_id:
        raise SystemExit(
            'V216 generation provenance preflight failed:\n'
            f'  experiment.id={exp_id}\n'
...
if ! grep -q "GradientNormOverflowRecovered" "$PROJECT_ROOT/trainer/abs_trainer.py"; then
  echo "V216 requires the already-installed V10 stable finite-gradient norm fallback in trainer/abs_trainer.py" >&2
  exit 2
fi
python "$PROJECT_ROOT/tools/test_local_frame_actuator.py"
...
for rel in [
    ...
    'train.py',
    'scripts/train/run_R41_R43_v216.sh',
]:
...
if pc_mode != 'representation_only':
    raise SystemExit(f'V216 requires pair_coordinate.mode=representation_only; got {pc_mode!r}')
cc_mode=str(cc.get('mode','') or '').strip().lower()
if cc_mode != 'local_frame_fullatom_affine':
    raise SystemExit('V216 requires coordinate_controller.mode=local_frame_fullatom_affine; '+repr(cc_mode))
frame=str(geom.get('frame','') or '').strip().lower()
if frame != 'diffab_n_ca_c_local':
    raise SystemExit(f'V216 requires geometry.frame=diffab_n_ca_c_local; got {frame!r}')
...
if local_scale <= 0 or not math.isclose(local_scale, flow_scale, rel_tol=0.0, abs_tol=1e-12):
    raise SystemExit('V216 coordinate-scale contract failed: local_coordinate_scale must equal r05.r3.coordinate_scaling')
...
if sl_source not in allowed:
    raise SystemExit(f'V216 invalid smooth-lDDT prediction_source={sl_source!r}')
...
print(
    ...
    'rigid_head_init=zero sidechain_local_residual_init=zero '
    'backbone_channels=N,CA,C,O fixed_context=restored_each_stage '
    'rotation_gradient=end_to_end no_edge_distance_times_scalar=1 '
