...
# ============================================================
# AbFlow v78: endpoint-parameterized hierarchical Score--Flow stage
# ============================================================
...
# pseudo-coordinate direct velocity carrier are used. Hierarchical analytic Score--Flow
# is evaluated from the SAME E01/E02 checkpoint with MODE=test_scoreflow, so the
# Flow--Score coupling costs no redundant 200-epoch training run.
...
if [[ "$MODE" == "test_scoreflow" ]]; then
  case "$EXP_ID" in
    E01_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT|E02_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT_INTERFACE_TIME)
      SAMPLER_MODE=r3_scoreflow
      ;;
    *)
      echo "test_scoreflow is only defined for E01/E02 stochastic endpoint checkpoints." >&2
      exit 2
      ;;
  esac
fi
...
if [[ "$MODE" == "test" || "$MODE" == "test_scoreflow" ]]; then
  CKPT=${4:-}
  RESULT_DIR=${5:-}
...
if [[ "$MODE" != "train" ]]; then
  ...
  echo "Test ScoreFlow: bash $0 test_scoreflow <E01|E02 EXP_ID> <GPU_ID> <CKPT> <RESULT_DIR> [TEST_JSON]"
  echo "Attach current training auto-eval: bash $0 attach_eval <EXP_ID> <GPU_ID|auto> [EVAL_GPU_ID|auto]"
  echo "v78 EXP_ID: E00_PCS_RC_LC_R1_ENDPOINT_FM_BASE, E01_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT, E02_PCS_RC_LC_R1_STOCHASTIC_ENDPOINT_INTERFACE_TIME"
  exit 2
fi
